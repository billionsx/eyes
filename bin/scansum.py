...
def by_scale(frames) -> dict:
    """Скругления отдельно у кадров точного канона и у пересчитанных."""
    out = {}
    for x in frames:
        sc = x.get("scale")
        if sc is None:
            continue
        key = "точный канон" if abs(sc - round(sc)) < 1e-6 else "пересчитан"
        b = out.setdefault(key, {"кадров": 0, "карточек": 0, "скруглённых": 0,
                                 "масштабы": set()})
        b["кадров"] += 1
        b["масштабы"].add(round(float(sc), 4))
        for s in (x.get("surfaces") or []):
            if s.get("role") != "card":
                continue
            b["карточек"] += 1
            if (s.get("radius_pt") or 0) > 0.5:
                b["скруглённых"] += 1
    for b in out.values():
        b["масштабы"] = sorted(b["масштабы"])[:6]
    return out
...
def summarize(frames_dir: Path, scan_path: Path) -> dict:
    ...
    out = {
        ...
        # РЕШАЮЩИЙ ОПЫТ. 124 карточки из 148 дали радиус РОВНО ноль — дуги
        # снялись и оказались прямыми. Объяснений два, и они требуют разного:
        # либо у Apple на этих экранах углы правда прямые, либо кадр прошёл
        # пересчёт размера у раздатчика и скругление размылось до потери.
        # Различить их можно числом: кадры ТОЧНОГО канона (масштаб ровно 3.0)
        # пересчёта не проходили. Если скругления есть только у них — виноват
        # пересчёт; если ни у кого — углы прямые на самом деле.
        "радиус_по_точности_масштаба": by_scale(ok),
    }
...
def court() -> int:
    ...
    _bs = by_scale([
        {"scale": 3.0, "surfaces": [{"role": "card", "radius_pt": 12.0},
                                    {"role": "card", "radius_pt": 0.0}]},
        {"scale": 2.9923, "surfaces": [{"role": "card", "radius_pt": 0.0}]}])
    chk("опыт разделяет точный канон и пересчитанный кадр",
        _bs["точный канон"]["скруглённых"] == 1
        and _bs["пересчитан"]["скруглённых"] == 0
        and _bs["пересчитан"]["кадров"] == 1)
    chk("масштабы названы поимённо — видно, что именно сравнивалось",
        _bs["пересчитан"]["масштабы"] == [2.9923])
