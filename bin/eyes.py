...
def cmd_selftest(root: Path) -> int:
    ...
    print("SELFTEST · замерщик не выдаёт свою слепоту за отсутствие кадров")
    import scansum as _ss
    _chain = {"1_файлов_добыто": 50, "2_видит_замерщик": 50,
              "3_записей_в_замере": 0, "4_принято": 0, "5_поверхностей": 0}
    check("ломаю → красный: 50 файлов видны, записей ноль — звено обрыва "
          "названо ЗАМЕРЩИКОМ, а не кадрами",
          _ss.break_link(_chain).startswith("замер"))
    check("файлов нет вовсе — названа добыча, а не замерщик",
          _ss.break_link({**_chain, "1_файлов_добыто": 0,
                          "2_видит_замерщик": 0}).startswith("добыча"))
    check("прочитал и не принял — названы кадры",
          _ss.break_link({**_chain, "3_записей_в_замере": 50}).startswith("кадры"))
    check("замерщик, не принявший НИ ОДНОГО кадра, возвращает отказ, "
          "а не пустой список",
          "НЕ ПРИНЯЛ НИ ОДНОГО" in
          (root / "bin" / "geoscan.py").read_text(encoding="utf-8"))
    check("причины отказа сохраняются рядом с замером, а не только печатаются",
          "rejected.json" in (root / "bin" / "geoscan.py").read_text(encoding="utf-8"))
    ...
    _cd = Path(_tf4.mkdtemp(prefix="eyes-cert-"))
    _reg = _cd / "REGISTER.md"
    _reg.write_text(
        "| месяц | скор | грейд | файл | размер | sha256 | выдан |\n"
        "|---|---|---|---|---|---|---|\n"
        "| 2026-07 | 83.7 | C | `2026-07.html` | 2707 | `abc…` | ts |\n",
        encoding="utf-8")
    _w = _ce.vision_conflicts(_cd, "2026-08", "аааа1111")
    check("выдача без записанного отпечатка объявлена НЕ сравнимой "
          "(незнание ≠ совпадение)",
          len(_w) == 1 and _w[0][2] == "не записан")
    check("ломаю → красный: имя файла не принимается за отпечаток "
          "(старая строка реестра уже 9 частей)",
          _w[0][2] != "2026-07.html")
    _reg.write_text(
        "| месяц | скор | грейд | зрение | файл | размер | sha256 | выдан |\n"
        "|---|---|---|---|---|---|---|---|\n"
        "| 2026-07 | 83.7 | C | `аааа1111` | `2026-07.html` | 1 | `abc…` | ts |\n"
        "| 2026-06 | 90.0 | B | `бббб2222` | `2026-06.html` | 1 | `def…` | ts |\n",
        encoding="utf-8")
    _w2 = _ce.vision_conflicts(_cd, "2026-08", "аааа1111")
    check("чиню → зелёный: выдача с ТЕМ ЖЕ отпечатком сравнима и в список "
          "не попадает",
          [m for m, _, _ in _w2] == ["2026-06"])
    check("надпись о несравнимости стоит на самом документе, а не в "
          "примечании к реестру",
          "НЕ СРАВНИМЫ" in _ce._vis_block("аааа1111", _w2)
          and "сравнимы" in _ce._vis_block("аааа1111", []))
    check("без отпечатка документ говорит, что оснований для сравнения нет",
          "нельзя" in _ce._vis_block("", []))
    check("оценка НЕ подкручена: формула осталась объявленной",
          _ce.score_of({"strict": 0, "live": 0, "verify_diverg": 0,
                        "report": 774, "files": 118}) == 65.3)
    ...
    check("суд пересборки зелёный: определённость, снос, восстановление",
          rb_mod.court() == 0)
    check("измеренная база НЕ объявлена выводимой — её нельзя пересобрать",
          "tokens.json" not in {n for n, _ in rb_mod.DERIVED})
    check("выводимое совпадает с источником: правок руками нет",
          rb_mod.check() == [])
    ...
    # Проверяется РАЗБОР, а не конкретные цифры. Прежняя редакция пришпиливала
    # «8 из 30» — числа живого замера. Любая честная перемера ломала суд, и
    # суд начинал требовать держать в базе устаревшее число. Испытание,
    # наказывающее за уточнение замера, работает против департамента.
    # Проверяется НЕПРОТИВОРЕЧИВОСТЬ разбора живой дыры, а не её цифры: текст
    # улики меняется с каждым уточнением замера, и суд не вправе это запрещать.
    check("разбор живой дыры непротиворечив: наличие не выдаётся за "
          "закрытие",
          bool(_rc) and (_rc[0]["need"] is None or _rc[0]["have"] is None
                         or _rc[0]["have"] < _rc[0]["need"]))
    _syn = _needs.holes.__globals__
    _ev = "🕳 скруглённых карточек 8; Для закрытия нужно 30 скруглённых"
    check("потребность НЕ читается как наличие: «нужно 30» и «карточек 8» "
          "разобраны врозь",
          _syn["RE_NEED"].search(_ev).group(1) == "30"
          and _syn["RE_HAVE"].search(_syn["RE_NEED"].sub(" ", _ev)).group(1) == "8")
