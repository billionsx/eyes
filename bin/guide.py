...
GUIDE = {
    ...
    "AE20": ("Капса в объявлении начертания.",
             "Убрать text-transform: uppercase. Заголовок группы идёт Title Case — капса у Apple отсутствует как приём, это наследие iOS 12. Нужен акцент — берите вес и цвет метки, не заглавные буквы.",
             ["typography.caps_lock"]),
    "AE17": ("Поверхность задана одним значением на обе темы.",
             "Дать поверхности ПАРУ: значение для светлой и для тёмной темы. "
             "Механизм любой — @media (prefers-color-scheme), атрибут "
             "data-theme, переменная темы. Apple прямо предупреждает: "
             "значения системных цветов меняются от выпуска к выпуску, и "
             "одно жёсткое значение на обе темы устаревает молча.",
             ["surfaces.allow"]),
    "AE18": ("Разделитель тоньше замеренной линии.",
             "Довести толщину разделителя до замеренной: линия тоньше неё исчезает на экране 1x и на печати — граница списка пропадает там, где она нужнее всего. Полпикселя это не тонкость, а отсутствие.",
             ["separator.width_pt"]),
    "AE19": ("Кегль задан жёстким px — интерфейс не переживёт Dynamic Type.",
             "Перевести кегли текстовых ролей на rem (или em/clamp) и "
             "привязать корень к системному кеглю. Пользователь, увеличивший "
             "текст в настройках, обязан увидеть увеличенный текст, а не "
             "обрезанный.",
             ["typography.role_sizes_pt"]),
}
...
def court():
    ...
    chk("наставление есть на КАЖДОЕ правило департамента",
        set(GUIDE) == {f"AE{i}" for i in range(1, 21)})
