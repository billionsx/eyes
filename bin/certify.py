...
# ВЕРСИЯ ФОРМУЛЫ. Стоит в сертификате и в реестре: выданное вчера обязано
# оставаться читаемым после правки шкалы.
FORMULA = 2

# Плотность долга вместо его объёма.
#
# Родословная (02.08.2026): шкала была ИНВЕРТИРОВАНА. Долг советника входил
# абсолютным числом, поэтому проект на 10 файлах с 50 находками (5 находок на
# файл) получал 97.5·A, а проект на 1000 файлов с 500 находками (0.5 на файл,
# вдесятеро чище) получал 75.0·C. Шкала награждала малый и грязный проект и
# наказывала большой и чистый. Для внешней оценки это не «неустойчивость», а
# обратный знак: сертификат сообщал покупателю противоположное правде.
#
# Насыщающая форма: штраф = DENS_MAX · d/(d+1), где d — находок на файл.
# Монотонна по долгу, монотонна по размеру, ограничена сверху. Ни один
# проект не проваливается ниже DENS_MAX только из-за объёма советника.
#
# 🕳 DENS_MAX выбран СОГЛАШЕНИЕМ, а не выведен из замера. Департамент обязан
# это называть: невыведенная постоянная есть дыра, а не закон (ЗКН-Э001).
# Калибровка: выбрана так, чтобы уже выданный грейд клиента не переехал.
DENS_MAX = 40.0
...
def debt_density(report: int, files: int) -> float:
    """Находок советника на просмотренный файл. Пустой обход плотности не даёт."""
    return report / max(1, files)


def score_of(c: dict) -> float:
    d = debt_density(c["report"], c.get("files", 0))
    dens_penalty = DENS_MAX * d / (d + 1.0)
    return round(max(0.0, 100.0 - W["strict"] * c["strict"]
                     - W["live"] * c["live"] - W["verify"] * c["verify_diverg"]
                     - dens_penalty), 1)
...
def run(project_root: Path, pdf: bool = False) -> dict:
    ...
    score = score_of(c)
    # Куда лечь документу. По умолчанию — в департамент. Но приватный проект
    # департамент забрать не может, не получив ключа клиента, а ключ, который
    # можно не выдавать, не выдают. Поэтому сертификат умеет считаться В САМОМ
    # ПРОЕКТЕ, его собственным прогоном, и ложиться туда же: документ живёт у
    # владельца, департамент ключа не держит. Формула, реестр и отпечатки —
    # те же самые, считает один и тот же орган.
