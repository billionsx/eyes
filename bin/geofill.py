...
def apply_to_base(dec: dict, base_path: Path = None) -> int:
    ...
    # Строка состояния базы обязана называть ЧИСЛО, и число это берётся ИЗ
    # САМОЙ БАЗЫ, а не из итогов текущего прогона.
    #
    # Родословная (05.08.2026). Сначала строка говорила «ни одно 🕳 не закрыто»
    # при пяти закрытых — соврала общим словом. Правка заменила слово на число
    # закрытий ЭТОГО ПРОГОНА — и соврала снова, в другую сторону: добыча кадров
    # из App Store не дала согласия ни по одной величине, прогон закрыл ноль, и
    # база объявила «закрыто замером 0 значений», имея шесть измеренных чисел.
    #
    # Утверждение о себе обязано считаться по себе. Итог прогона — не состояние
    # базы, и подменять одно другим значит врать дважды на одном месте (Э002).
    def _closed_in(o):
        if isinstance(o, dict):
            return sum(_closed_in(v) for v in o.values())
        if isinstance(o, (int, float)) and not isinstance(o, bool):
            return 1
        if isinstance(o, list):
            return 1 if o and not any(isinstance(x, str) and "🕳" in str(x)
                                      for x in o) else 0
        return 0
    ...
    # замером» значит присвоить конвейеру чужую работу — мелкая ложь того же
    # рода, что и две предыдущие на этом же месте.
    #
...
def court() -> int:
    ...
    check("строка состояния базы называет и общее число значений, и число "
          "закрытий конвейера — своё от чужого отделено",
          "значений стоит" in _d["base"]
          and "закрыто замерным конвейером" in _d["base"])
    # Итог прогона — не состояние базы. Прогон, закрывший НОЛЬ, не вправе
    # объявить пустой базу, в которой числа есть.
    _b2 = Path(__import__("tempfile").mkdtemp()) / "base.json"
    _b2.write_text(json.dumps({"base": "каркас", "a": {"x": 12.0, "y": 16.0},
                               "m": {"z": "🕳 замерить"}}, ensure_ascii=False),
                   encoding="utf-8")
    apply_to_base({"frames": 1, "closed": {}, "holes": {}}, _b2)
    check("ломаю → красный: прогон без закрытий НЕ объявляет базу пустой — "
          "значения считаются по самой базе",
