...
def clone(name: str, repo: str, branch: str = "main") -> tuple:
    ...
    if r.returncode != 0:
        ...
        if ("not found" in low or "could not read username" in low
            ...
            # ФОРМУЛИРОВКА ИМЕЕТ ЦЕНУ. Первая редакция говорила «репозитория
            # нет либо он закрыт», и обе половины стояли рядом как равные. Их
            # прочитали как первую: 05.08.2026 департамент доложил основателю,
            # что `billionsx/ethnomir-app` не существует. Он существует —
            # приватный и вне области ключа. GitHub отвечает на такой 404, тем
            # же кодом, что и на несуществующий, и различить их со стороны
            # НЕВОЗМОЖНО.
            #
            # Поэтому вперёд ставится самая частая причина, а не самая
            # тревожная: ключ. Диагноз, допускающий два чтения, будет прочитан
            # в том, которое пугает, — и решение примут не то.
            last = (f"ключ департамента не видит {repo}. Самая частая причина: "
                    f"репозиторий ПРИВАТНЫЙ и не входит в область ключа — "
                    f"GitHub отвечает на такой 404, тем же кодом, что и на "
                    f"несуществующий, и различить их со стороны нельзя. "
                    f"Проверять надо ОБЛАСТЬ КЛЮЧА, а не наличие репозитория")
