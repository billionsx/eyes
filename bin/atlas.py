...
def order_frontier(frontier: list, fw: dict, probe: int = PROBE) -> list:
    ...
    def rank(pid):
        ...
        # ПРЕДМЕТ ГЛАВНЕЕ ФОРМЫ. Форма — уточнение ВНУТРИ фреймворка, а не
        # множитель поверх него.
        #
        # Родословная (02.08.2026, ошибка моя): форма была введена множителем,
        # и слабый по предмету фреймворк обходил сильный за счёт вида адреса.
        # Замер прогона: 1279 прочитанных страниц — 100% статьи (отбор формы
        # сработал), но 269 из них RealityKit, 0.21 строки со страницы. Урожай
        # упал до 0.28 — НИЖЕ заглушек символов (0.45). Статья про трёхмерную
        # сцену остаётся статьёй не по предмету: форма говорит, насколько
        # страница подробна, и ничего не говорит о том, о чём она.
        #
        # Ключ сортировки — пара: сперва предмет, затем форма. Ничего не
        # удаляется, порядок детерминирован (ЗКН-Э001).
