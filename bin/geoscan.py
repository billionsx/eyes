...
# Допуск на ширину кадра, полученного не с устройства, а от раздатчика
# изображений. Объявлен долей, а не пикселями: у широкого кадра тот же
# промах в пикселях значит меньше.
NEAR_CANON = 0.004        # 0.4% — на 1170px это ±4px


def scale_of(width_px: int, near: float = NEAR_CANON):
    """Масштаб и ширина экрана в точках. Не сошлось → (None, None).

    ТОЧНОЕ РАВЕНСТВО И БЛИЗОСТЬ — РАЗНЫЕ ВЕЩИ, И ОБЕ ЗАКОННЫ.
    ------------------------------------------------------------------
    Родословная (05.08.2026). Правило требовало РОВНОГО совпадения
    `pt × scale`, и это было верно, пока кадры приходили с устройства. Добыча
    из App Store вскрыла второй случай: раздатчик изображений отдаёт по
    запрошенной ширине не ровно её — на 1170 приходит 1167, 1169, 1200. Из 50
    добытых кадров замерщик отверг 39 за промах в один-три пикселя, и
    департамент решил, что по витрине мерить нельзя. Мерить можно; выбрасывал
    он сам.

    Подобрать ближайший канон «на глаз» по-прежнему нельзя — это подмена. Но
    отвергать кадр, чья ширина отличается на 0.3%, значит терять улику ради
    буквы. Поэтому масштаб считается ДЕЙСТВИТЕЛЬНЫЙ: `ширина / pt`, дробный, а
    не канонический. Ни один пиксель не выдумывается и не досочиняется —
    меняется только делитель, которым пиксели переводятся в точки.

    Условие близости строгое: промах меньше 0.4% ширины И только к одному
    канону. Кадр, равно близкий к двум канонам, отвергается: выбор между
    двумя одинаково правдоподобными ответами есть догадка (ЗКН-Э001).
    """
    for sc in SCALES:
        for pt in SCREEN_PT:
            if pt * sc == width_px:
    ...
    for sc in SCALES:
        for pt in SCREEN_PT:
            want = pt * sc
            if want and abs(width_px - want) <= near * want:
                hits.append((abs(width_px - want), width_px / pt, pt))
    if len(hits) != 1:
        return None, None
    _, exact, pt = hits[0]
    return exact, pt
...
def court() -> int:
    ...
    check("канон сходится ровно: 1170px = 390pt × 3", scale_of(1170) == (3.0, 390))
    check("чиню → зелёный: 1167px принят с ДЕЙСТВИТЕЛЬНЫМ масштабом, "
        "а не подогнан к канону",
        scale_of(1167)[1] == 390 and abs(scale_of(1167)[0] - 1167 / 390) < 1e-9)
    check("ломаю → красный: 1200px вне допуска — отвергнут, а не подобран",
        scale_of(1200) == (None, None))
    check("равно близкий к двум канонам отвергается: выбор между двумя "
        "правдоподобными ответами есть догадка",
        scale_of(1176, near=0.02) == (None, None))
    check("допуск объявлен долей, а не пикселями", 0 < NEAR_CANON < 0.01)
