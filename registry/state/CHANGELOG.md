...
### · 02.08.2026 · AE18 · первое новое правило из замера
Точный замер охвата (база подменена записывающей — линт сам показал, к чему обращается): линт трогает **8 разделов базы из 20**. Незакрытыми с настоящим потенциалом оказались `separator`, `rows`, `tabbar`, `glyphs` — измеренные числа, которых не охраняет ни одно правило. Измерение без принуждения есть знание, а не надзор.

Объявлено **AE18 · РАЗДЕЛИТЕЛЬ**: линия тоньше замеренной `separator.width_pt = 1` (📐 замер 217 кадров, база прямо говорит «НЕ 0.5: полпикселя исчезает»). Универсально, проверяемо, число берётся ИЗ БАЗЫ — суд доказывает, что подмена базы меняет вердикт, то есть число не зашито в код. Доказано в обе стороны на живых фикстурах. На клиенте найдено 5 живых нарушений, все адреса сличены с исходником вручную.

Два дефекта, вскрытых по дороге:
- **Правило молча зависело от соседа.** Оно стояло внутри ворот `if "AE14" in rules or "AE15" in rules` — у клиента, не включившего AE14/AE15, новое правило не работало бы вовсе и молчало вместо красного. Ворота теперь перечисляют все правила, судящие по блокам css.
- **Единого реестра номеров правил не существовало.** Я объявил правило как AE16 — а AE16 уже принадлежит монитору («заливка под активным пунктом нижней навигации»), AE17 — линту («пара тем»). Два закона под одним номером сломали бы храповик и сертификат: долг одного списывался бы на другое. Правило переименовано в AE18, заведён `registry/RULES.md`, суд проверяет уникальность номеров и что каждое включённое паспортом правило в реестре есть.
- 02.08.2026 · **я выложил в main несобирающийся `bin/eyes.py`.** Грубое авторазрешение конфликта при спешке оставило маркеры в коде органа — суд на main перестал запускаться вовсе, и узнал об этом департамент только от меня. Реестр был защищён проверкой целости json, а собственный код органов — нет: защита была несимметричной. Введена проверка, что каждый орган разбирается питоном и не несёт маркеров конфликта; внесён урок **У7** свода. Конфликт разрешён объединением обеих сторон — обе несли новые проверки, и «победа» одной стороны потеряла бы половину суда.
